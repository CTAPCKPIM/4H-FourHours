...
describe("FourHours: ", () => {
  ...
  it("Should be created token.", async () => {
    await expect(
      sellerOfNFTs.connect(user).createAndBuy("https://www.japancars.ua/", { value: _value })
    )
      .to.emit(sellerOfNFTs, "Successfully")
      .withArgs(2, anyValue);
  });
  ...
  it("Successfully withdrawal.", async () => {
    ...
    await expect(sellerOfNFTs.connect(owner).withdrawal(owner.address))
      .to.emit(sellerOfNFTs, "Withdrawal")
  ...
  it('Should be the error: "ERR: address/no exist".', async () => {
    ...
    await expect(
      _sellerOfNFTs.connect(user).createAndBuy("https://www.japancars.ua", { value: _value })
  ...
  it('Should be the error: "ERR: price".', async () => {
    ...
    await expect(
      sellerOfNFTs.connect(user).createAndBuy("https://www.japancars.ua")
    ).to.be.revertedWith(message);
  });
